import pool from './db';
import { getTrafficReadTableSql } from './trafficSource';
...
const MODEL_LOOKBACK_MINUTES = Number(
  process.env.MODEL_WINDOW_LOOKBACK_MINUTES || MODEL_BUCKET_MINUTES * MODEL_WINDOW_SIZE * 3
);
...
function floorToBucketTime(value: Date | string) {
...
function toBucketKey(value: Date) {
  return value.toISOString();
}

export function getModelBucketMinutes() {
  return MODEL_BUCKET_MINUTES;
}

export function getModelWindowSize() {
  return MODEL_WINDOW_SIZE;
}

async function getLatestSnapshot(nodeIds: string[]) {
  const trafficTable = getTrafficReadTableSql();
...
export async function buildModelWindow(nodeIds: string[]) {
  const trafficTable = getTrafficReadTableSql();
  const [rows]: any = await pool.query(
  ...
  const bucketMap = new Map<string, Record<string, { sum: number; count: number }>>();
  for (const row of (rows || []) as TrafficRow[]) {
