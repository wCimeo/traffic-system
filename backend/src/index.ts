...
const INCIDENT_NODE_NAMES: Record<string, string> = {
  A1: '天府大道-锦城大道路口',
  B2: '益州大道-锦城大道路口',
  C3: '成华大道-杉板桥路口',
  D4: '天府大道-华阳立交路口',
...
function buildIncidentDescription(node: { id: string; name: string }, type: { value: string; detail: string }, severity: number) {
    ...
    await pool.query(
      `UPDATE incidents
       ...
       WHERE status IN ('active', 'resolved', 'ignored')
         AND (handler_id IS NULL OR handler_id = '')`,
      [roleIds[0]]
    );
  }
}
...
function buildRecommendation(predictedSpeed: number) {
  if (predictedSpeed >= 40) {
    return { recommendation: 'recommended', level: 'good' as const };
  }
  if (predictedSpeed < 25) {
    return { recommendation: 'reroute', level: 'bad' as const };
  }
  return { recommendation: 'caution', level: 'normal' as const };
}
...
async function getLatestTrafficMap(nodeIds = NODE_IDS) {
  ...
  return new Map<string, any>((rows || []).map((row: any) => [row.node_id, row]));
}

function parseNodeList(rawValue: unknown, fallback: string[] = []) {
  const rawText = Array.isArray(rawValue) ? rawValue.join(',') : String(rawValue || '');
  const parsed = rawText
    .split(',')
...
async function ensurePredictionsTableMigration() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS predictions (
      ...
      model_bucket_minutes TINYINT NOT NULL DEFAULT ${getModelBucketMinutes()},
  ...
  const addColumnIfMissing = async (name: string, definition: string) => {
    if (!columnSet.has(name)) {
      await pool.query(`ALTER TABLE predictions ADD COLUMN ${name} ${definition}`);
      columnSet.add(name);
    }
  };
  ...
  await addColumnIfMissing('model_bucket_minutes', `TINYINT NOT NULL DEFAULT ${getModelBucketMinutes()}`);
  await pool.query(`
...
async function persistPredictionSnapshot(flaskData: any, generatedAt: Date) {
...
app.get('/api/predict/latest', async (req, res) => {
  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT p.node_id, p.predicted_speed, p.predicted_at, p.horizon_minutes, p.target_at, p.source_table, p.model_bucket_minutes
       FROM predictions p
       INNER JOIN (
    ...
    await pool.query(
      `UPDATE incidents
       SET status = ?,
           ...
           END
       WHERE id = ?`,
      [nextStatus, nextHandlerId, nextStatus, nextStatus, req.params.id]
    );
...
app.delete('/api/incidents/:id', requireAuth, async (req: AuthenticatedRequest, res) => {
...
app.post('/api/incidents/mock-seed', async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      const handler =
        status === 'reported' && Math.random() < 0.45
          ? ''
...
app.get('/api/dashboard/chart', async (req, res) => {
  ...
  try {
    ...
    const [predictionRows]: any = await pool.query(
      `SELECT p.node_id, p.predicted_speed, p.predicted_at, p.horizon_minutes, p.target_at, p.source_table, p.model_bucket_minutes
       FROM predictions p
       INNER JOIN (
